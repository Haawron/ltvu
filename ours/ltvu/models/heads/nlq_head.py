    ...
    def _generate_points(self):
        ...
        for l, stride in enumerate(self.fpn_strides):
            reg_range = torch.as_tensor(
                self.regression_range[l], dtype=torch.float)  # [2]
            fpn_stride = torch.as_tensor(stride, dtype=torch.float)
            points = torch.arange(0, self.max_seq_len, stride)[:, None]  # [T_f=T/2^f, 1]
            # add offset if necessary (not in our current model)
            if self.use_offset:
                points += 0.5 * stride
            # pad the time stamp with additional regression range / stride
            reg_range = reg_range[None].repeat(points.shape[0], 1)  # [T_f, 2]
            fpn_stride = fpn_stride[None].repeat(points.shape[0], 1)  # [T_f, 1]
            # size: T x 4 (ts, reg_range, stride)
            points_list.append(torch.cat((points, reg_range, fpn_stride), dim=1))  # [T_f, 4]
...
class NLQHead(nn.Module):
    def __init__(self, in_dim, max_v_len, feature_grid_stride_sec=2):
        super().__init__()
    ...
    def forward(
        ...
        feat,              # required
        mask=None,
        training=True,     # required
        gt_segments=None,  # required
        gt_labels=None,
        v_lens=None,
    ):
        """
        ...
        """
        B, D, T = feat.size()
        mask = mask if mask is not None else torch.ones_like(feat, dtype=torch.bool, device=feat.device)

        masks = [mask]
        feats = [feat]
        # masks: 1 x [B, 1, T] -> fpn_masks: F x [B, 1, T_i]
        fpn_feats, fpn_masks = self.neck(feats, masks)

        points = self.point_generator(fpn_feats)
        out_cls_logits = [x.permute(0, 2, 1) for x in self.cls_head(fpn_feats, fpn_masks)]  # F x [B, T_i, K=1]
        out_offsets = [x.permute(0, 2, 1) for x in self.reg_head(fpn_feats, fpn_masks)]  # (B, T, #cls*2)
        fpn_masks = [x.squeeze(1) for x in fpn_masks]  # F x [B, T_i]
        ...
        if training:
            ...
            # records, keys each: 'segments', 'scores', 'labels'
            results = self.inference(points, fpn_masks, out_cls_logits, out_offsets, 1)
            return results
    ...
    def label_points(self, points, gt_segments, gt_labels, num_classes):
        ...
        num_levels = len(points)  # F
        concat_points = torch.cat(points, dim=0)  # [ΣT_f, 4]

        gt_cls, gt_offset = [], []
        # loop over each video sample
        for gt_segment, gt_label in zip(gt_segments, gt_labels):  # over B
        ...
        return gt_cls, gt_offset  # B x [ΣT, K=1], B x [ΣT, 2]

    @torch.no_grad()
    def label_points_single_video(self, concat_points, gt_segment, gt_label, num_classes):
        # concat_points : [ΣT, 4], each (t, *regression range=[0, 10000], fpn_stride=1)
        # gt_segment : [N_j=1, 2]
        # gt_label : [N_j=1, K=1], one_hot encoded
        num_pts = concat_points.shape[0]  # ΣT from [ΣT, 4]
        num_gts = gt_segment.shape[0]  # N_j = 1 from [N_j=1, 2]
        ...
        lens = gt_segment[:, 1] - gt_segment[:, 0]  # [N_j=1,]
        lens = lens[None, :].repeat(num_pts, 1)  # [ΣT, N_j=1]

        # compute the distance of every point to each segment boundary
        # auto broadcasting for all reg target-> F T x N x 2
        gt_segs = gt_segment[None].expand(num_pts, num_gts, 2)  # [N_j=1, 2] -> [ΣT, N_j=1, 2]
        left = concat_points[:, 0, None] - gt_segs[:, :, 0]  # [ΣT, N_j=1], how much time passed since the GT start
        right = gt_segs[:, :, 1] - concat_points[:, 0, None]  # [ΣT, N_j=1], how much time left to the GT end
        reg_targets = torch.stack((left, right), dim=-1)  # [ΣT, N_j=1, 2]

        if self.train_center_sample == 'radius':
            # center of all segments F T x N
            center_pts = 0.5 * (gt_segs[:, :, 0] + gt_segs[:, :, 1])  # [ΣT, N_j=1]
            # center sampling based on stride radius
            # compute the new boundaries:
            # concat_points[:, 3] stores the stride
            t_mins = \
                center_pts - concat_points[:, 3, None] * self.train_center_sample_radius  # [ΣT, N_j=1]
            t_maxs = \
                center_pts + concat_points[:, 3, None] * self.train_center_sample_radius  # [ΣT, N_j=1]
            ...
            center_seg = torch.stack(
                (cb_dist_left, cb_dist_right), -1)  # [ΣT, N_j=1, 2]

            inside_gt_seg_mask = center_seg.min(-1)[0] > 0  # [ΣT, N_j=1], if both +
        else:
            # inside an gt action
            inside_gt_seg_mask = reg_targets.min(-1)[0] > 0

        # limit the regression range for each location
        max_regress_distance = reg_targets.max(-1)[0]  # [ΣT, N_j=1]

        inside_regress_range = torch.logical_and(
            (max_regress_distance >= concat_points[:, 1, None]),
            (max_regress_distance <= concat_points[:, 2, None])
        )  # [ΣT, N_j=1], if both + and the position is not too skewed
        # ...
        # (x) s<-- whatever -->t<---------- more than 10000 ---------->e, ... (x N_j)
        # (o) s<---- whatever ---->t<-------- less than 10000 -------->e, ... (x N_j)
        # ...
        ...
        # lens = [
        #   (durations of each GT action)
        #   [96,  inf, inf],
        #   [96,  inf, inf],
        #   [inf, inf, inf],
        #   [inf, 83,  inf],
        #   (though seg1, seg2 have the same class label, the result one_hot element will be at most 1)
        #   [inf, 83,  83 ],
        #   [inf, inf, 83 ],
        #   ...
        # ]  # [ΣT, N_j], N_j = 3 in this case
        ...
        # ※ Same as no-op if N_j == 1
        min_len, min_len_inds = lens.min(dim=1)  # [ΣT,], [ΣT,], same as lens if N_j == 1
        min_len_mask = torch.logical_and(
            (lens <= (min_len[:, None] + 1e-3)),  # all true if N_j == 1
            (lens < float('inf'))
        ).to(reg_targets.dtype)  # [ΣT, N_j=1], float

        # cls_targets: [ΣT, K=1]; reg_targets: [ΣT, 2]
        # gt_label_one_hot = F.one_hot(gt_label, num_classes).to(reg_targets.dtype)
        gt_label_one_hot = gt_label.to(reg_targets.dtype)  # [N_j=1, K=1]
        # valid one-hots
        cls_targets = min_len_mask @ gt_label_one_hot  # [ΣT, N_j=1] @ [N_j=1, K=1] -> [ΣT, K=1]
        # to prevent multiple GT actions with the same label and boundaries
        cls_targets.clamp_(min=0.0, max=1.0)  # [ΣT, K=1], acts as a valid mask for each t if K == 1

        # the corresponding segment for each point (not checked for validity)
        reg_targets = reg_targets[range(num_pts), min_len_inds]  # [ΣT, N_j=1, 2] -> [ΣT, 2]
        # normalization based on stride
        reg_targets /= concat_points[:, 3, None]
        # cls_targets = [                       # reg_targets = [
        #   [0, 0, 0, 1],                       #   [  1,  97],
        #   [0, 0, 0, 1],                       #   [  1,  97],
        #   [0, 0, 0, 0],  # invalid point      #   [  1,  97],  # should be regarded as a garbage value
        #   [1, 0, 0, 0],                       #   [105, 188],
        #   [1, 0, 0, 0],                       #   [105, 188],
        #   [1, 0, 0, 0],                       #   [153, 236],
        #   ...                                 #   ...
        # ]  # [ΣT, K], K = 4 in this case      # ]  # [ΣT, 2]
        return cls_targets, reg_targets
    ...
    def losses(
        ...
        pos_mask = torch.logical_and((gt_cls.sum(-1) > 0), valid_mask)  # [B, ∑T], foreground samples
        ...
        gt_target = gt_cls[valid_mask]  # [#Pos, K=1]
        ...
        gt_offsets = torch.stack(gt_offsets)[pos_mask]  # B x [∑T, 2] -> [B, ∑T, 2] -> [#Pos, 2]
        if num_pos == 0:
    ...
    def inference(
            self,
            points, fpn_masks,
            out_cls_logits, out_offsets, num_classes, v_lens=None
    ):
    ...
    def inference_single_video(
        ...
        for cls_i, offsets_i, pts_i, mask_i in zip(
            ...
            pred_prob: torch.Tensor = (cls_i.sigmoid() * mask_i.unsqueeze(-1)).flatten()  # [T_i x K=1]
            ...
            pt_idxs = torch.div(topk_idxs, num_classes, rounding_mode='floor')  # [2000], in [0, T_i)
            cls_idxs = torch.fmod(topk_idxs, num_classes)   # [2000], in [0, K=1)
    ...
    def postprocessing(self, results):
        ...
        for results_per_vid in results:
            ...
            if segs.shape[0] > 0:  # FIXME: 이거 초로 바꿔줘야 되나?
                # segs_sec = (segs * stride + 0.5 * nframes) / fps
                segs_sec = self.feature_grid_stride_sec * segs
