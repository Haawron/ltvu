...
from ...models.heads.nlq_head import NLQHead
...
class FrozenInTime(BaseModel):
    def __init__(self,
                ...
                projection_dim=256,
                load_checkpoint='cache_dir/egovlp-config-removed.pth',
                projection='minimal',
...
class TextOnlyFrozenInTime(BaseModel):
    def __init__(self, device=None, **kwargs):
        super().__init__(**kwargs)
        model = FrozenInTime(device=device, **kwargs)
    ...
    def forward(self, text_batch: list[dict[str, torch.Tensor]]):
        z = self.model.compute_text(text_batch)
...
if __name__ == "__main__":
    ...
    model = TextOnlyFrozenInTime()
    dataset = EgoNLQwithNarrationsDataset()
    dataloader = torch.utils.data.DataLoader([dataset[0]], batch_size=1)
