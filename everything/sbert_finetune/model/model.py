...
class SentenceGroundingModel(nn.Module):
    ...
    def compute_loss(self, sim, gt_mat: torch.Tensor, loss_fn='multi-pos'):
        ...
        if loss_fn == 'multi-pos':
            ...
            loss = F.binary_cross_entropy_with_logits(sim, gt_mat, reduction='none')
        elif loss_fn == 'bce':
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.alpha_loss_q_cap > 0:
            ...
            loss_q_cap = alpha * loss_q_cap.mean()
            loss = loss + loss_q_cap
        ...
        if self.alpha_loss_cap_cap > 0:
            ...
            loss_cap_cap = alpha * loss_cap_cap.mean()
            loss = loss + loss_cap_cap
